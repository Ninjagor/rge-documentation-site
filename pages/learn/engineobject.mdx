...
An important thing to note is that RGE.js follows an `Object Oriented` paradigm. This means that any time you want to perform practically any action, you would do so via an object.
...
Great job! You have successfully instantiated the `Engine` object!
...
<Callout type="error" emoji="️❗️">
  Make sure to delete `rge` once you have stopped the Engine
</Callout>
...
#### Use Case
`setTickFunction()` is used to set a function to be the "tick", which is run every animation frame.
...
#### Use Case
`addKeyPressAction()` is used to register keyPress actions. RGE automatically will listen for those key presses, and will perform the according actions.
